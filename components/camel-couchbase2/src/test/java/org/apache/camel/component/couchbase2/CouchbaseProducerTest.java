...
public class CouchbaseProducerTest {
    ...
    public void before() throws Exception {
        ...
        producer = new CouchbaseProducer(endpoint, couchbaseBucket, 0, 0);
    ...
    public void testPersistToLowerThanSupported() throws Exception {
        producer = new CouchbaseProducer(endpoint, couchbaseBucket, -1, 0);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testPersistToHigherThanSupported() throws Exception {
        producer = new CouchbaseProducer(endpoint, couchbaseBucket, 5, 0);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testReplicateToLowerThanSupported() throws Exception {
        producer = new CouchbaseProducer(endpoint, couchbaseBucket, 0, -1);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testReplicateToHigherThanSupported() throws Exception {
        producer = new CouchbaseProducer(endpoint, couchbaseBucket, 0, 4);
    }

    @Test
    public void testMaximumValuesForPersistToAndRepicateTo() throws Exception {
        try {
            producer = new CouchbaseProducer(endpoint, couchbaseBucket, 4, 3);
    ...
    public void testExpiryTimeIsSet() throws Exception {
    	when(result.content()).thenAnswer(new Answer<Object>() {
            @Override
        ...
        when(couchbaseBucket.setAdd(anyString(), anyInt(), any(), any(), any())).thenReturn(of);
        ...
        verify(couchbaseBucket).setAdd(anyString(), eq(expiry), any(), any(), any());
    }
